...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationWillFinishLaunching(_ notification: Notification) {
        menubarController.buttonInit()
    }
    
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        menubarController.startRunning()
        buttonActionInit()
        popoverHandler()
        eventMonitorHandler()
    }
    ...
    func buttonActionInit() {
        menubarController.statusBar.statusItem.button?.action = #selector(togglePopover(_:))
    }
    
    func eventMonitorHandler() {
        eventMonitor = EventMonitor(mask: [.leftMouseDown, .rightMouseDown]) { [weak self] event in
            if let strongSelf = self, strongSelf.popover.isShown {
                if self?.boxStatus.isPin == false {
    ...
    func showPopover(sender: Any?) {
        if let button = menubarController.statusBar.statusItem.button {
    ...
    lazy var persistentContainer: NSPersistentContainer = {
         ...
         */
        let container = NSPersistentContainer(name: "Box42")
